...
// (function ($, window, document, undefined) {
    ...
    var updateHistogram = function (selectedRange, sliderMin, rangePerBin, histogramName, sliderName) {
        ...
        $('#minimum-price-slider').val(leftValue.toLocaleString('en-US', {style: 'currency',currency: 'USD',}));
        $('#maximum-price-slider').val(rightValue.toLocaleString('en-US', {style: 'currency',currency: 'USD',}));

        $("#" + sliderName + "-value").html(leftValue.toLocaleString('en-US', {style: 'currency',currency: 'USD',}) + " - " + rightValue.toLocaleString('en-US', {style: 'currency',currency: 'USD',}));
        ...
        init: function (options) {
            ...
            $("#" + sliderName).slider({
              ...
              updateHistogram(event.value, self.options.sliderRange[0], rangePerBin, histogramName, sliderName);
                ...
                livewirePriceRangeCall(event.value);
              updateHistogram(event.value, self.options.sliderRange[0], rangePerBin, histogramName, sliderName);
            });
